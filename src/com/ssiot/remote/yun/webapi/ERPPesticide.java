...
public class ERPPesticide extends WebBase {
    private static final String tag = "ERPPesticide";
    private String MethodFile = "Pesticides.asmx";

    public void Delete() {

    }

    public List<ERPPesticideModel> GetPesticide(int mainUserid) {
        HashMap<String, String> params = new HashMap<String, String>();
        params.put("where", " ProductBatchID In (select ID from cms2016.dbo.ERP_ProductBatch where UserID=" + mainUserid + ") ");//TODO
        String txt = exeRetString(MethodFile, "GetFertilizer", params);//王生辉忘改名
        return parse(txt);
    }
    
    public int Save(ERPPesticideModel m) {
        HashMap<String, String> params = new HashMap<String, String>();
        params.put("ID", "" + m._id);
    ...
    private List<ERPPesticideModel> parse(String str) {
        List<ERPPesticideModel> models = new ArrayList<ERPPesticideModel>();
        try {
            ...
            for (int i = 0; i < items.getLength(); i++) {
                ERPPesticideModel cType = new ERPPesticideModel();
        ...
        for (ERPPesticideModel m : models){
